...
export class DrawService {
  ...
  async initDraw(): Promise<void> {
    ...
    this.drawControl = new LA.Control.Draw({
      ...
      draw: {
        ...
        polygon:      { allowIntersection: false, shapeOptions: { color: '#1565c0', fillColor: '#1565c0', fillOpacity: 0.2 } },
        circle:       { shapeOptions: { color: '#1565c0', fillColor: '#1565c0', fillOpacity: 0.2 } },
        rectangle:    { shapeOptions: { color: '#1565c0', fillColor: '#1565c0', fillOpacity: 0.2 } },
        marker:       true,
    ...
    map.on(LA.Draw.Event.EDITED, (e: any) => {
      e.layers.eachLayer((layer: any) => {
        ...
        if (id) {
          const updated = this.layerToGeoJSON(layer, layer._drawType ?? 'polygon');
          if (updated) {
            this.features.update((fs) =>
              fs.map((f) => f.properties.id === id
  ...
  updateFeatureProperties(id: string, props: Partial<GisFeature['properties']>): void {
    this.features.update((fs) =>
      fs.map((f) =>
        f.properties.id === id
  ...
  exportGeoJSON(): GisFeatureCollection {
    return {
      ...
      metadata: { name: 'OSM Angular GIS Export', createdAt: new Date().toISOString() },
    };
  }
  ...
  async importGeoJSON(geojson: GisFeatureCollection): Promise<void> {
    ...
    const geoLayer = (L as any).geoJSON(geojson, {
      ...
      onEachFeature: (feature: any, layer: any) => {
        ...
        const props = { ...feature.properties, id, createdAt: feature.properties?.createdAt ?? new Date().toISOString() };
        layer._gisId = id;

        const gisFeature: GisFeature = { type: 'Feature', geometry: feature.geometry, properties: props };
    ...
    try { map.fitBounds(geoLayer.getBounds(), { padding: [20, 20] }); } catch { /* empty */ }
  }
  ...
  private layerToGeoJSON(layer: any, type: DrawToolType): GisFeature | null {
    try {
      ...
      if (type === 'circle') {
        ...
        return {
          type: 'Feature',
          geometry: { type: 'Point', coordinates: [center.lng, center.lat] } as any,
          properties: {
            id, name: this.getFeatureName(type),
            drawType: type, radius,
  ...
  private getFeatureName(type: DrawToolType): string {
    const names: Record<DrawToolType, string> = {
      marker: 'Marcador', circle: 'Círculo', polygon: 'Polígono',
      polyline: 'Línea', rectangle: 'Rectángulo', circlemarker: 'Punto',
    };
